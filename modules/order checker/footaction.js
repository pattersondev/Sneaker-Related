...
async function orderChecker () {
	...
	try {
		const response = await got('https://www.footaction.com/api/v3/session?timestamp=' + Date.now(), {
			responseType: 'json',
			cookieJar: cookieJar
		}).json()
		...
		const newRes = await got.post('https://www.footaction.com/api/users/orders/status?timestamp=' + Date.now(), {
			responseType: 'json',
			cookieJar: cookieJar,
			headers: {
				...
				'pragma': 'no-cache',
				'referer': 'https://www.footaction.com/order/search.html',
