...
async function orderChecker () {
	...
	try {
		const response = await got('https://www.eastbay.com/api/v3/session', {
			responseType: 'json',
			cookieJar: cookieJar
		}).json()
		...
		const newRes = await got.post('https://www.eastbay.com/api/users/orders/status?timestamp=' + Date.now(), {
			responseType: 'json',
			cookieJar: cookieJar,
			headers: {
				...
				'pragma': 'no-cache',
				'referer': 'https://www.eastbay.com/order/search.html',
