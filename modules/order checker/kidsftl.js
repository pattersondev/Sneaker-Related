...
async function orderChecker () {
	...
	try {
		const response = await got('https://www.kidsfootlocker.com/api/v3/session?timestamp=' + Date.now(), {
			responseType: 'json',
			cookieJar: cookieJar
		}).json()
		...
		const newRes = await got.post('https://www.kidsfootlocker.com/api/users/orders/status?timestamp=' + Date.now(), {
			responseType: 'json',
			cookieJar: cookieJar,
			headers: {
				...
				'pragma': 'no-cache',
				'referer': 'https://www.kidsfootlocker.com/order/search.html',
