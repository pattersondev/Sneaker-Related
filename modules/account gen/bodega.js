const puppeteer = require('puppeteer-extra')
const StealthPlugin = require('puppeteer-extra-plugin-stealth')
puppeteer.use(StealthPlugin())

var firstName = 'Sam'
var lastName = 'Patterson'
var email = 'hksallfksajfnasmlfda@gmail.com'
var password = 'TestPassword123'
const fakeUserAgent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:53.0) Gecko/20100101 Firefox/53.0'

async function bodegaAcc () {
  const browser = await puppeteer.launch({
    headless: true,
    executablePath: 'C:\\Program Files (x86)\\Google\\Chrome\\Application\\chrome.exe'
    })
    const page = await browser.newPage()
    await page.evaluateOnNewDocument(fakeUserAgent => {
      let open = window.open
  
      window.open = (...args) => {
      let newPage = open(...args)
      Object.defineProperty(newPage.navigator, 'userAgent', {get: () => fakeUserAgent})
      return newPage
      }
  
      window.open.toString = () => 'function open() { [native code] }'
          
      }, fakeUserAgent)
      
  
    await page.setUserAgent(fakeUserAgent)
    await page.goto('https://bdgastore.com/account/register', 
    { waitUntil: 'domcontentloaded',
      referer: 'https://bdgastore.com/account/'
    })
    await page.waitForSelector('#FirstName')
    await page.focus('#FirstName')
    await page.keyboard.type(firstName)
    await page.focus('#LastName')
    await page.keyboard.type(lastName)
    await page.focus('#Email')
    await page.keyboard.type(email)
    await page.focus('#CreatePassword')
    await page.keyboard.type(password)
    await page.click('#create_customer > p > input')

    if(page.url() === 'https://bdgastore.com/') {
      console.log('success')
    } else if (page.url() === 'https://bdgastore.com/challenge') {
      console.log('captcha')
    } else {
      console.log('error')
    }
}

bodegaAcc()